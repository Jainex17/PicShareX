    ...
    return (
        <section className="fileupload container mx-auto pt-8 px-8 bg-white dark:bg-[#2a2a2f]">
            <div {...getRootProps({ className: `border-dashed border-4 ${images.length > 0 ? "border-blue-500" :'border-gray-500'} flex flex-col justify-center items-center cursor-pointer` })}>
                {
                    images.length > 0 ? <img src={URL.createObjectURL(images[0])} className='w-full h-72 object-cover' alt="preview" draggable={false} />
                :
            ...
            </div>
            {
                ...
                <>
                    ...
                    <div className='mb-10'>
                        {/* images preview */}
                        <h1 className='text-3xl font-bold text-gray-900 dark:text-gray-100 mb-6'>Images Preview</h1>

                        <div className='flex gap-6 justify-center flex-wrap'>
                            {images.map((image, index) => (
                                <div key={index} className="flex flex-col border border-[#b47517] rounded-md select-none">
                                    <img src={URL.createObjectURL(image)} alt="preview image" draggable={false} className="hover:brightness-75 transition  flex justify-center items-center object-cover w-64 h-64" />

                                    <div className='flex justify-between bg-white dark:bg-[#333]'>
                                        
                                    <p className="text-gray-600 dark:text-white text-xs py-2 px-3">
                                        {image.name && image.name.length > 22 ? (image.name.slice(0, 22) + "...") : image.name}
                                    </p>
                                    <p className="text-gray-600 dark:text-white text-xs py-2 px-3">
